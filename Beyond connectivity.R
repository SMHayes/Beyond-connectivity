...
#set up plotting
    ...
    cols = rainbow(n.cluster)
    
    patch.cols = seq(from=1,to=patch)
    
    i.clust = 1
    while(i.clust<=n.cluster)
    {
      patch.cols[patch.cluster==i.clust] = cols[i.clust]
...
#plot network structure
    ...
    plot.igraph(NM,vertex.color=patch.cols,vertex.size=40,edge.width=3)
    
#plot pred abundance vs prey abundance
    
    plot(prey[start.time:sim.length,1],pred[start.time:sim.length,1],pch=19,col=patch.cols[1],xlim=c(min(prey[start.time:sim.length,]),max(prey[start.time:sim.length,])),ylim=c(min(pred[start.time:sim.length,]),max(pred[start.time:sim.length,])),ylab="Pred",xlab="Prey")
    
    i = 2
    while(i<=ncol(pred)){
      
      points(prey[start.time:sim.length,i],pred[start.time:sim.length,i],col=patch.cols[i],pch=19)  
...
#plot prey abundance vs time
    ...
    plot(seq(start.time,sim.length),prey[start.time:sim.length,1],type="l",col=patch.cols[1],ylim=c(min(prey[start.time:sim.length,]),max(prey[start.time:sim.length,])),ylab="Abundance",xlab="Time",main="Prey")
    
    i = 2
    while(i<=ncol(prey)){
      
      lines(seq(start.time,sim.length),prey[start.time:sim.length,i],col=patch.cols[i])  
...
#plot pred abundance vs time
    ...
    plot(seq(start.time,sim.length),pred[start.time:sim.length,1],type="l",col=patch.cols[1],ylim=c(min(pred[start.time:sim.length,]),max(pred[start.time:sim.length,])),ylab="Abundance",xlab="Time",main="Pred")
    
    i = 2
    while(i<=ncol(pred)){
      
      lines(seq(start.time,sim.length),pred[start.time:sim.length,i],col=patch.cols[i])  
